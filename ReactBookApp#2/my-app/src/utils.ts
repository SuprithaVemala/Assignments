...
function searchBooks(searchText:string,choice:string){
  ...
  switch(choice){
    ...
    case "price": let [min, max] = searchText.split(" ");
                  return books.filter((book:any) => book.Price >= Number(min) && book.Price <= Number(max));
...
export { getBooksFromStorage, addBooksToStorage, newnow,getBookByID ,deleteBook,searchBooks};
