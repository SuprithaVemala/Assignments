function starComponent() {
  ...
  <link rel="stylesheet" href="././scss/starStyle.css">
  <div class="fa outer">
  ...
  class demo extends HTMLElement {
    constructor() {
      ...
      let newVal =
        ((parseFloat(rating) * parseFloat(5)) / parseFloat(limit)) * 15;
...
async function booklist(term,val) {
  ...
  if(term==="author" || term==="title" || term==="rating"){
    uri+=`?${term}=${val}`
  }
  if(term==="id"){
    uri+=`/${val}`
  ...
  if(term==="price"){
    ...
    uri+=`?${term}=${pricelimits[0]}&${term}=${pricelimits[1]}`
  }
  ...
  for (let b of books) {
    ...
    td3.innerHTML=`
    <star-rating title="${rating.toString()}" rating="${rating.toString()}"></star-rating>
    `
